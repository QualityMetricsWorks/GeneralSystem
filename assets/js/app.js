import {$,toast,esc} from './utils.js?v=1.5.0.8';
import {state,getClient,getPart,getRun} from './state.js?v=1.5.0.8';
import * as api from './db.js?v=1.5.0.8';
import * as ui from './ui.js?v=1.5.0.8';
import {getFilteredHistory} from './ui.js?v=1.5.0.8';
import {metricsForRuns,copqForEvent} from './metrics.js?v=1.5.0.8';
import {initTraceability,renderBarcode,printBarcode} from './traceability.js?v=1.5.0.8';
import {initI18n,applyLanguage,tr} from './i18n.js?v=1.5.0.8';
...
function exportExcel(){
 ...
 const production=hist.production.map(r=>{
  const m=metricsForRuns([r]);
  return {
   Fecha:r.date,
   Turno:r.shift||'',
   Cliente:getClient(r.clientId)?.name||'',
   NP:getPart(r.partId)?.number||'',
   Operacion:state.operations.find(x=>x.id===r.operationId)?.code||'',
   Maquina:state.machines.find(x=>x.id===r.machineId)?.code||'',
   Produccion:r.produced,
   Scrap:m.scrap,
   Yield:m.yieldRate,
   PPM:m.ppm,
   COPQ:m.copq
 ...
 const quality=hist.quality.map(e=>{
  const r=getRun(e.runId),p=getPart(r?.partId);
  return {
   Fecha:r?.date||'',
   Turno:e.shift||r?.shift||'',
   Cliente:getClient(r?.clientId)?.name||'',
   NP:p?.number||'',
   Operacion:state.operations.find(x=>x.id===r?.operationId)?.code||'',
   Defecto:state.defects.find(x=>x.id===e.defectId)?.name||'',
   Cantidad:e.quantity,
 ...
 const downtime=hist.downtime.map(e=>{
  const r=getRun(e.runId),reason=state.downtimeReasons.find(x=>x.id===e.reasonId);
  return {
   Fecha:r?.date||'',
   Turno:e.shift||r?.shift||'',
   Cliente:getClient(r?.clientId)?.name||'',
   ...
   Maquina:state.machines.find(x=>x.id===r?.machineId)?.code||'',
   Paro:reason?.name||'',
   Tipo:e.eventType||reason?.downtimeType||'',
   Razon:e.reason||'',
   Comentarios:e.notes||'',
