import {state} from './state.js?v=1.5.0.8';
let db=null;
...
export async function loadAll(){
 ...
 state.parts=p.data.map(x=>({id:x.id,clientId:x.client_id,number:x.number,description:x.description||'',costPerPiece:Number(x.cost_per_piece||0),scrapCostPerPiece:Number(x.scrap_cost_per_piece||0),currency:x.currency||'USD',createdAt:x.created_at||''}));
 state.operations=o.data.map(x=>({id:x.id,partId:x.part_id,code:x.code,name:x.name,isFinishGood:Boolean(x.is_finish_good),createdAt:x.created_at||''}));
...
export async function insertPart(x){const row=check(await db.from('part_numbers').insert({...common(),client_id:x.clientId,number:x.number,description:x.description||null,cost_per_piece:x.costPerPiece||0,scrap_cost_per_piece:x.scrapCostPerPiece||0,currency:x.currency||'USD'}).select().single());localAdd('parts',row);return row}
export async function deletePart(id){const r=await db.from('part_numbers').delete().eq('id',id);check(r);localRemove('parts',id)}

export async function updatePart(id,x){const row=check(await db.from('part_numbers').update({description:x.description||null,cost_per_piece:x.costPerPiece||0,scrap_cost_per_piece:x.scrapCostPerPiece||0,currency:x.currency||'USD'}).eq('id',id).select().single());localReplace('parts',id,row);return row}
...
export async function insertOperation(x){const row=check(await db.from('operations').insert({...common(),part_id:x.partId,code:x.code,name:x.name,is_finish_good:Boolean(x.isFinishGood)}).select().single());localAdd('operations',row);return row}
export async function setOperationFinishGood(id,value){const row=check(await db.from('operations').update({is_finish_good:Boolean(value)}).eq('id',id).select().single());localAdd('operations',row);return row}
