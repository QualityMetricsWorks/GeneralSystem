import {$,number,money,percent,esc,dispositionLabel} from './utils.js?v=1.5.0.8';
import {state,getClient,getPart,getOperation,getDefect,getRun,getMachine,getPersonnel,getDowntimeReason,cycleTimesForPart,partsForClient,operationsForPart,defectsForPart,eventsForRun,machinesForPart,partsForMachine} from './state.js?v=1.5.0.8';
import {metricsForRuns,filteredRuns,defectPareto,topProducts,scrapQtyForRun,copqForEvent,oeeMetrics} from './metrics.js?v=1.5.0.8';
let charts={};
...
export function renderSelects(){
 ...
 ['scrapHistoryShift','downtimeHistoryShift'].forEach(id=>{const el=$(id);if(!el)return;const v=el.value;el.innerHTML=`<option value="">Todos los turnos</option>`+state.shiftSchedules.map(x=>`<option value="${esc(x.code)}">${esc(`${x.code} · ${x.name}`)}</option>`).join('');if([...el.options].some(o=>o.value===v))el.value=v});
}
let historyFiltered={production:[],quality:[],downtime:[]};
export function getFilteredHistory(){return historyFiltered}
export function renderHistory(){
 ...
 const shiftScrap=$('scrapHistoryShift')?.value||'';
 const shiftDown=$('downtimeHistoryShift')?.value||'';
 ...
 if(prodBody){
  ...
  historyFiltered.production=rows;
 ...
 if(scrapBody){
  const rows=state.scrapEvents.slice().sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt))).filter(e=>{
   const r=getRun(e.runId),p=getPart(r?.partId),c=getClient(r?.clientId)||getClient(p?.clientId),o=getOperation(r?.operationId),d=getDefect(e.defectId);
   const shift=e.shift||r?.shift||'';
   if(shiftScrap&&shift!==shiftScrap)return false;
   return `${r?.date||''} ${shift} ${r?.lotNumber||''} ${c?.name||''} ${p?.number||''} ${o?.code||''} ${d?.name||''} ${e.disposition||''} ${e.reason||''} ${e.notes||''}`.toLowerCase().includes(qScrap);
  });
  historyFiltered.quality=rows;
  scrapBody.innerHTML=rows.map(e=>{const r=getRun(e.runId),p=getPart(r?.partId),c=getClient(r?.clientId)||getClient(p?.clientId),o=getOperation(r?.operationId);return `<tr><td>${esc(r?.date||'—')}</td><td>${esc(e.shift||r?.shift||'—')}</td><td>${esc(c?.name||'—')}</td><td>${esc(p?.number||'—')}</td><td>${esc(o?.code||'—')}</td><td>${esc(getDefect(e.defectId)?.name||'—')}</td><td>${number(e.quantity)}</td><td>${esc(dispositionLabel(e.disposition))}</td><td>${esc(e.reason||e.notes||'—')}</td><td>${money(copqForEvent(e),p?.currency||'USD')}</td><td></td></tr>`}).join('')||'<tr><td colspan="11" class="empty-state">Sin resultados.</td></tr>';
 }
 if(downBody){
  const rows=state.downtimeEvents.slice().sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt))).filter(e=>{
   const r=getRun(e.runId),p=getPart(r?.partId),c=getClient(r?.clientId)||getClient(p?.clientId),m=getMachine(r?.machineId),rs=getDowntimeReason(e.reasonId);
   const shift=e.shift||r?.shift||'';
   if(shiftDown&&shift!==shiftDown)return false;
   return `${r?.date||''} ${shift} ${r?.lotNumber||''} ${c?.name||''} ${p?.number||''} ${m?.code||r?.machine||''} ${rs?.name||''} ${e.eventType||''} ${e.notes||''}`.toLowerCase().includes(qDown);
  });
  historyFiltered.downtime=rows;
  downBody.innerHTML=rows.map(e=>{const r=getRun(e.runId),p=getPart(r?.partId),c=getClient(r?.clientId)||getClient(p?.clientId),m=getMachine(r?.machineId),rs=getDowntimeReason(e.reasonId);return `<tr><td>${esc(r?.date||'—')}</td><td>${esc(e.shift||r?.shift||'—')}</td><td>${esc(c?.name||'—')}</td><td>${esc(p?.number||'—')}</td><td>${esc(m?.code||r?.machine||'—')}</td><td>${esc(rs?.name||'—')}</td><td>${esc(e.eventType||'unplanned')}</td><td>${esc(e.notes||'—')}</td><td>${number(e.minutes)}</td><td></td></tr>`}).join('')||'<tr><td colspan="10" class="empty-state">Sin resultados.</td></tr>';
 }
}
...
function renderKpiComparisons(){
 ...
 setKpiComparison('kpiCopq',cm.copqPercent,pm.copqPercent,true);
}
...
function renderCustomDashboard(kind,runs,downtime){
 ...
 items.forEach(x=>{
   const scopedRuns=runs.filter(r=>(!x.clientId||r.clientId===x.clientId)&&(!x.partId||r.partId===x.partId));
   const scopedDowntime=downtime.filter(e=>{const r=getRun(e.runId);return (!x.clientId||r?.clientId===x.clientId)&&(!x.partId||r?.partId===x.partId)});
   const d=customData(kind,x.metric,scopedRuns,scopedDowntime),cid=`customDashCanvas-${kind}-${x.id}`;
...
export function renderPartDetail(){
 const p=getPart(state.selectedPartId),empty=$('partEmptyState'),detail=$('partDetail');if(!p){empty.hidden=false;detail.hidden=true;return}empty.hidden=true;detail.hidden=false;$('partDetailNumber').textContent=p.number;$('partDetailDescription').textContent=p.description||'Sin descripción';$('editPartDescription').value=p.description||'';$('editPartCost').value=p.costPerPiece||0;$('editPartScrapCost').value=p.scrapCostPerPiece||0;$('editPartCurrency').value=p.currency||'USD';$('partDetailClient').textContent=getClient(p.clientId)?.name||'—';$('partDetailCost').textContent=money(p.costPerPiece,p.currency);$('partDetailScrapCost').textContent=money(p.scrapCostPerPiece,p.currency);
